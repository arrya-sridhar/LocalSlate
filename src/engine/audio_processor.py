...
class WhisperProcessor:
    ...
    def transcribe(self, audio_path: Path) -> str:
        ...
        if self.model is None:
            logging.warning("Whisper model not loaded. Falling back to mock transcription.")
            import time
            time.sleep(2.0)
            ...
            try:
                audio_path.unlink()
                logging.info(f"Mock Transcription: Securely deleted raw audio {audio_path.name}")
            except Exception as e:
        ...
        try:
            audio_path.unlink()
            logging.info(f"Securely deleted raw audio post-processing: {audio_path.name}")
        except Exception as e:
            logging.error(f"Failed to delete audio file: {e}")
